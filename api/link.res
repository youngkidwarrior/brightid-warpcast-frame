type hubRpcClient
@module("@farcaster/hub-nodejs")
external getSSLHubRpcClient: string => hubRpcClient = "getSSLHubRpcClient"

@val @scope(("process", "env"))
external hubUrl: option<string> = "HUB_URL"

let client = hubUrl->Option.map(getSSLHubRpcClient)

module FrameRequest = {
  include Request
  @get external query: t => option<'query> = "query"
}
...
let handler = async (req: FrameRequest.t, res: Response.t) => {
  ...
  //   <html>
  //     <head>
  //       <title>BrightID Link</title>
  //       <meta property="og:title" content="Link BrightID to Farcaster">
